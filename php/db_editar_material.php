...
$GLOBALS['id_material'] = $list['Id'];
$GLOBALS['doc_material'] = $Nome_doc;
...
function button(){
    ...
    $New_Nome_doc = $_FILES['doc']['name'];

    if(!isset($New_Nome_doc)){
        $New_Nome_doc = null;
    }

    if($New_Nome_doc != null){
        ...
        if (array_search($extensao, $_UP['extensoes']) === false) {
            ...
            $New_Nome_doc = str_replace($letra_a, "a", $New_Nome_doc);
            $New_Nome_doc = str_replace($letra_A, "A", $New_Nome_doc);
            $New_Nome_doc = str_replace($letra_o, "o", $New_Nome_doc);
            $New_Nome_doc = str_replace($letra_O, "O", $New_Nome_doc);
            $New_Nome_doc = str_replace($letra_e, "e", $New_Nome_doc);
            $New_Nome_doc = str_replace($letra_E, "E", $New_Nome_doc);
            $New_Nome_doc = str_replace($letra_u, "u", $New_Nome_doc);
            $New_Nome_doc = str_replace($letra_U, "U", $New_Nome_doc);
            $New_Nome_doc = str_replace($letra_c, "c", $New_Nome_doc);
            $New_Nome_doc = str_replace($letra_C, "C", $New_Nome_doc);
            $New_Nome_doc = str_replace($letra_i, "i", $New_Nome_doc);
            $New_Nome_doc = str_replace($letra_I, "I", $New_Nome_doc);

            if (move_uploaded_file($Documento, $_UP['pasta'] . $New_Nome_doc)) {
                $id = parse_url($_SERVER["REQUEST_URI"], PHP_URL_QUERY);
                $sql_edit = "UPDATE material SET Id = ".$GLOBALS['id_material'].", Titulo = '$Titulo', Descricao = '$Descricao', Data = '$Data', Autor = '$Autor', Nome_doc = '$New_Nome_doc', Link = '$Link', Categoria = '$Categoria'  where Id = ".$id."";
                $query_edit = mysqli_query($GLOBALS["conexao"], $sql_edit);
                
                unlink('documentos/'.$GLOBALS['doc_material']);
        ...
        $New_Nome_doc = $GLOBALS['doc_material'];
...
<body>
    <div class="tela">
        <fieldset class="field">
            ...
            <form method="post" enctype="multipart/form-data">
                ...
                <input type="file" name="doc" id="arquivo" class="btn" accept=".doc, .docx, .pdf"><br>

                <?php
                    if(substr($list["Nome_doc"], -4) == "docx" or substr($list["Nome_doc"], -3) == "doc"){
                        echo '
                            <button type="button" id="btn" class="ex"><a style="color: black; text-decoration: none;" href="http://localhost/faculdade/php/documentos/'.$list["Nome_doc"].'">Baixar Documento</a></button>
                        ';
                    }else{
                        echo'
    ...
    <div class="doc_view" id="doc" style="display: none;">  
        <?php
            if(substr($list["Nome_doc"], -4) == "docx" or substr($list["Nome_doc"], -3) == "doc"){
                echo '
                    <button id="apagar" class="ex"><a style="color: white; text-decoration: none;" href="http://localhost/faculdade/php/documentos/'.$Nome_doc.'">Baixar Documento</a></button>
                ';
            }else{
                echo'
                    <iframe src="http://localhost/faculdade/php/documentos/'.$Nome_doc.'"
